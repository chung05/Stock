...
async function run() {
  try {
    console.log("🚀 啟動【主力成分股 v2 - 核心去重雙 API 智慧自適應版】同步程序...");
    ...
    // 💡 物理級 Set() 強行去重，確保名單永遠維持精準的 180 檔，不重複堆疊
    ...
    // 3. 🛡️ 【下午16:00盤後防護機制】判斷今日資料是否 Ready
    const now = new Date();
    ...
    // 4. 逐檔增量同步核心（100% 完好對齊原本可執行的資料欄位，無任何自增髒欄位）
    if (startDate <= endDate) {
      for (let i = 0; i < stockIds.length; i++) {
        ...
        try {
          ...
          if (rowsToUpsert.length > 0) {
            // 💡 智慧對齊：移除了非專案欄位 updated_at，確保 Upsert 順暢無阻
