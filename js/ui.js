...
import { state, getValIgnoreCase, decodeMultiDimensionSignal } from './config.js';
  ...
  setTimeout(() => {
    ...
    if (state.currentMacdFilter !== 'ALL') {
      filteredStocks = filteredStocks.filter(item => {
        ...
        // 💡 智慧修正：呼叫新版多維度解碼函式，並用 .includes 檢查是否符合選定的篩選器代號
        const matchedSignals = decodeMultiDimensionSignal(myChips);
        return matchedSignals.includes(state.currentMacdFilter);
      });
    }
