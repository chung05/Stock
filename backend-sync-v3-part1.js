...
async function run() {
  try {
    console.log("🚀 【牛牛大帳本 v3 - Part1】啟動 (全量價量 + 法人籌碼 + 核心指標演算)...");

    console.log("📥 正在從雲端唯一真理母名單 stock_targets 載入擴充後的 231 檔名單...");
    ...
    if (taipeiHour < 16) {
      console.log("🕒 台灣時間未滿 16:00，安全機制啟動：同步終點限制在【昨天】。");
    ...
    if (startDate > endDate) {
      console.log(`💡 大帳本已是最新狀態（至 ${endDateStr}），Part1 暫無須補件。`);
      return;
    }

    console.log(`📅 Part1 實質增量區間: ${startDateStr} 至 ${endDateStr}`);
    const commonHeaders = { 'accept': 'application/json', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' };

    // 逐檔同步價量與籌碼 (完全剝離資券接口，控制在 2 接口調用)
    for (let i = 0; i < stockIds.length; i++) {
      ...
      console.log(`[Part1 下載] (${i + 1}/${stockIds.length}) ${sId}`);
      ...
      try {
          ...
          res.data.data.forEach(row => {
            ...
            if (!dateMap[d]) {
              dateMap[d] = { 
                ...
                open: 0, max: 0, min: 0, trading_volume: 0 // 資券欄位在此留空，靜待 Part2 融合
              };
            }
        ...
        if (rowsToUpsert.length > 0) {
          // 強制設定複合唯一鍵約束對齊，就地 Upsert
          const { error: upsertErr } = await supabase.from('stock_chips_daily').upsert(rowsToUpsert, { onConflict: 'stock_id,date' });
          if (upsertErr) throw upsertErr;
        }
      } catch (err) {
        console.error(`❌ Part1 下載 ${sId} 錯誤: ${err.message}`);
      }
      await sleep(150); // 溫和冷卻
    }

    console.log("💡 [Part1 第二步] 數據基礎對齊，立刻啟動核心指標重算大腦，防禦網頁死白...");
    await calculateAndWriteBackIndicators(stockIds);

    console.log("🎉 【Part1 價量籌碼大底】同步與全指標計算順利通車！");
  } catch (error) {
    console.error("💥 Part1 全局致命錯誤:", error.message);
  }
}

// 完整繼承 MainStock 高精度 28 欄位遞迴大腦重算模組
  ...
  for (let i = 0; i < stockList.length; i++) {
    ...
    try {
      ...
      for (let j = 0; j < totalLen; j++) {
        ...
        // 🌟 核心防禦：繼承並保留原本就存在格子裡的資券舊數據 (防範被覆蓋為 0)
        rowUpdates.push({
          stock_id: sId, date: targetDay.date, price: targetDay.price, open: targetDay.open, max: targetDay.max, min: targetDay.min, trading_volume: targetDay.trading_volume, change_value: targetDay.change_value, f_buy: targetDay.f_buy, f_sell: targetDay.f_sell, fd_buy: targetDay.fd_buy, fd_sell: targetDay.fd_sell, it_buy: targetDay.it_buy, it_sell: targetDay.it_sell, ds_buy: targetDay.ds_buy, ds_sell: targetDay.ds_sell, dh_buy: targetDay.dh_buy, dh_sell: targetDay.dh_sell,
          margin_buy: targetDay.margin_buy || 0, margin_sell: targetDay.margin_sell || 0, margin_balance: targetDay.margin_balance || 0, short_buy: targetDay.short_buy || 0, short_sell: targetDay.short_sell || 0, short_balance: targetDay.short_balance || 0,
          ma5: calculatedMA5, ma10: calculatedMA10, ma20: calculatedMA20, rsi14: calculatedRSI14, rsv: calculatedRSV, kd_k: calculatedK, kd_d: calculatedD, macd_dif: calculatedDif, macd_signal: calculatedMacdSignal, macd_osc: calculatedMacdOsc
        });
      }

      if (rowUpdates.length > 0) {
        await supabase.from('stock_chips_daily').upsert(rowUpdates, { onConflict: 'stock_id,date' });
        console.log(`[Part1 指標重算成功] ${sId}`);
      }
    } catch (singleErr) { console.error(`❌ Part1 演算異常: ${singleErr.message}`); }
    await sleep(60);
  }
}
run();
